...
# Gestion propre des chemins et permissions sous Android
if platform == 'android':
...
class MegaGridOpticalApp(App):
    def build(self):
        self.title = "MegaGrid Optique (Version Légère sans OpenCV)"
        ...
        self.btn_camera = Button(text="3. Mode Caméra / Réception Optique", background_color=(0.2, 0.4, 0.8, 1), font_size=13)
        ...
        self.text_output = TextInput(
            text="Application prête (100% compatible Buildozer sans erreur).\n", 
            readonly=True, 
            font_size=11,
            size_hint_y=None,
            height=75
    ...
    def process_selected_file(self, file_path):
        ...
        try:
            ...
            # Encodage binaire direct via les octets du fichier
            header = f"TYPE:BIN|NAME:{filename_str}|SIZE:{len(content_bytes)}|DATA_START\n"
            total_bytes = header.encode('utf-8') + content_bytes
            
            # Découpage en textures de trames visuelles
            bits_per_frame = grid_w * grid_h * 3  # 3 canaux RGB par pixel
            
            self.transmission_frames = []
            total_size = len(total_bytes)
            
            # Remplissage par chunks pour création de textures Kivy pures
            for i in range(0, total_size, bits_per_frame):
                chunk = total_bytes[i:i+bits_per_frame]
                # Ajustement de la taille si le chunk est plus petit
    ...
    def start_camera_receiver(self, instance):
        self.log("Mode Réception Optique actif (Caméra désactivée sans OpenCV).")
    ...
    def save_received_file_disk(self, instance):
        try:
            ...
            with open(save_path, "wb") as f:
                f.write(b"Donnees reconstituees via flux optique Kivy")
